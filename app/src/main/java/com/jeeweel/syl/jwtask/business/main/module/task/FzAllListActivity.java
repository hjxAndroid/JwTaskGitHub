...
public class FzAllListActivity extends JwListActivity {
                ...
                try {
                    if (mode == 0) {
                        ...
                        list = jCloudDB.findAllByWhere(Task.class,
                                "principal_code like " + StrUtils.QuotedStrLike(users.getUser_code()) + " ORDER BY create_time DESC limit " + pageStart + "," + pageEnd);
                        mListItems.clear();
                    } else {
                        setPage(false);
                        list = jCloudDB.findAllByWhere(Task.class,
