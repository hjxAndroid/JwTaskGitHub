...
public class JobAllListActivity extends JwListActivity {
                ...
                try {
                    if (mode == 0) {
                        ...
                        list = jCloudDB.findAllByWhere(Task.class,
                                "promulgator_code = " + StrUtils.QuotedStr(users.getUser_code()) + "ORDER BY create_time DESC  limit " + pageStart + "," + pageEnd);
                        mListItems.clear();
                    } else {
                        setPage(false);
                        list = jCloudDB.findAllByWhere(Task.class,
