...
public class OveredTaskListActivity extends JwListActivity {
                ...
                try {
                    if (mode == 0) {
                        ...
                        list = jCloudDB.findAllByWhere(Task.class,
                                "principal_code = "+StrUtils.QuotedStr(users.getUser_code())+" AND now_state < 3  and over_time < NOW() ORDER BY create_time ASC limit " + pageStart + "," + pageEnd);
                        mListItems.clear();
                    } else {
                        setPage(false);
                        list = jCloudDB.findAllByWhere(Task.class,
