...
public class ContantCommitActivity extends JwListActivity {
    ...
    public void initListViewController() {
        commonAdapter = new CommonAdapter<Taskcommit>(getMy(), mListItems, R.layout.item_job_list) {
            ...
            public void convert(ViewHolder helper, Taskcommit item) {
                ImageView iv_xz = helper.getImageView(R.id.iv_xz);
                iv_xz.setVisibility(View.GONE);
                ...
                try {
                    if (mode == 0) {
                        ...
                        taskcommits = jCloudDB.findAllByWhere(Taskcommit.class,
                                "task_code = " + StrUtils.QuotedStr(userdept.getDept_code()) + " limit " + pageStart + "," + pageEnd);
                        mListItems.clear();
                    } else {
                        setPage(false);
                        taskcommits = jCloudDB.findAllByWhere(Taskcommit.class,
        ...
        protected String doInBackground(String... params) {
            ...
            try {
                if (null != userdept) {
                    ...
                    taskcommit.setTask_code(userdept.getDept_code());
