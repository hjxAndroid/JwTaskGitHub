...
import com.jeeweel.syl.jwtask.business.main.module.basic.WelcomActivity;
...
public class AppShortCutUtil {
	 ...
	 * 
	 */
	public static void addNumShortCut(Context context, boolean isShowNum,
			String num, boolean isStroke) {
		Class<?> clazz = WelcomActivity.class;
		...
		if (count > 0) {
			if (Build.MANUFACTURER.equalsIgnoreCase("Xiaomi")) {
				// 小米
				sendToXiaoMi(context, clazz, num);
			} else if (Build.MANUFACTURER.equalsIgnoreCase("samsung")) {
				// 三星
				samsungShortCut(context, num);
			} else if (Build.MANUFACTURER.toLowerCase().contains("sony")) {
				sendToSony(context, clazz, num);
			} else {// 其他原生系统手机
				deleteRawShortCut(context, clazz);
				installRawShortCut(context, clazz, isShowNum, num, isStroke);
			}
		} else {
			deleteShortCut(context, clazz);
		}
	}

	private static void sendToSony(Context context, Class<?> clazz, String num) {
		boolean isShow = true;
		String number = num;
	 ...
	 * 
	 */
	private static void sendToXiaoMi(Context context, Class<?> clazz, String num) {
		String number = num;
		if (TextUtils.isEmpty(num)) {
			number = "";
		} else {
		...
		Log.e(TAG, "sendToXiaoMi....num:" + number);
		...
		try {
			NotificationCompat.Builder builder = new NotificationCompat.Builder(
					context);
			builder.setContentTitle("您有" + number + "个标未查看");
			builder.setTicker("您有" + number + "个标未查看");
			builder.setAutoCancel(true);
			builder.setSmallIcon(R.drawable.ic_logo);
			builder.setDefaults(Notification.DEFAULT_LIGHTS);
			Intent notificationIntent = null;
			notificationIntent = new Intent(context, WelcomActivity.class);
			...
			Log.e(TAG, "-----Exception-----:" + context.getPackageName() + "/"
					+ clazz.getName());
			e.printStackTrace();
			// miui 6之前的版本
			isMiUIV6 = false;
	...
	public static void samsungShortCut(Context context, String num) {
		...
		Log.e(TAG, "samsungShortCut....num:" + count);
	...
	public static void sonyShortCut(Context context, String num) {
		...
		localIntent.putExtra(
				...
				activityName);
		localIntent.putExtra(
				"com.sonyericsson.home.intent.extra.badge.MESSAGE", num);
		localIntent.putExtra(
				"com.sonyericsson.home.intent.extra.badge.PACKAGE_NAME",
				context.getPackageName());
		context.sendBroadcast(localIntent);
	...
	public static void installRawShortCut(Context context, Class<?> clazz,
			boolean isShowNum, String num, boolean isStroke) {
		...
		if (isStroke) {
			shortcutIntent.putExtra(
					...
					generatorNumIcon3(context, ((BitmapDrawable) context
							.getResources().getDrawable(R.drawable.ic_logo))
							.getBitmap(), isShowNum, num));
		} else {
			shortcutIntent.putExtra(
					Intent.EXTRA_SHORTCUT_ICON,
					generatorNumIcon2(context, ((BitmapDrawable) context
							.getResources().getDrawable(R.drawable.ic_logo))
							.getBitmap(), isShowNum, num));
		}
	...
	public static Bitmap generatorNumIcon(Context context, Bitmap icon,
		...
		if (isShowNum) {
			...
			if (numInt > 99) {// 超过99
				...
				int textWidth = (int) numPaint
						.measureText(num, 0, num.length());
				...
				canvas.drawCircle(iconSize - circleRadius - textWidth
						+ (10 * factor), circleCenter, circleRadius,
						leftCirPaint);

				// 绘制右边的圆形
				Paint rightCirPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
				rightCirPaint.setColor(Color.RED);
				canvas.drawCircle(iconSize - circleRadius, circleCenter,
						circleRadius, rightCirPaint);

				// 绘制中间的距形
				Paint rectPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
				rectPaint.setColor(Color.RED);
				RectF oval = new RectF(iconSize - circleRadius - textWidth
						+ (10 * factor), 2 * factor, iconSize - circleRadius,
						circleRadius * 2 + 2 * factor);
				canvas.drawRect(oval, rectPaint);

				// 绘制数字
				canvas.drawText(num,
						(float) (iconSize - textWidth / 2 - (24 * factor)),
						23 * factor, numPaint);
				...
				canvas.drawCircle(iconSize - 15 * factor, 15 * factor,
						15 * factor, inCirPaint);

				// 绘制数字
				canvas.drawText(num,
						(float) (iconSize - textWidth / 2 - 15 * factor),
						22 * factor, numPaint);
			}
		}
		return numIcon;
	...
	public static Bitmap generatorNumIcon3(Context context, Bitmap icon,
		...
		if (isShowNum) {
			...
			// 绘制数字
			canvas.drawText(num, (float) (iconSize - (backgroundWidth
					+ textWidth + 4 * strokeThickness) / 2), (22) * factor + 2
					* strokeThickness, numPaint);
		}
		return numIcon;
	}
	 ...
	 * @return
	 */
	public static Bitmap generatorNumIcon4(Context context, Bitmap icon,
			boolean isShowNum, String num) {
		...
		if (isShowNum) {
			...
			int backgroundWidth = textWidth > backgroundHeight ? (int) (textWidth
					+ 10 * factor + 2 * strokeThickness)
					: backgroundHeight;
			...
			drawable.setIntrinsicWidth(backgroundWidth);
			drawable.setBounds(0, 0, backgroundWidth, backgroundHeight);
			canvas.translate(iconSize - backgroundWidth - strokeThickness,
					2 * strokeThickness);
			drawable.draw(canvas);
	...
	public static boolean isAddShortCut(Context context) {
		...
		/*
		 * int versionLevel = android.os.Build.VERSION.SDK_INT; String AUTHORITY
		 * = "com.android.launcher2.settings"; //2.2以上的系统的文件文件名字是不一样的 if
		 * (versionLevel >= 8) { AUTHORITY = "com.android.launcher2.settings"; }
		 * else { AUTHORITY = "com.android.launcher.settings"; }
		 */
